...
*".  Includes much of utils.py from Norvig and Russell's Artificial
Intelligence: A Modern Approach.

"""

from __future__ import generators
import operator, math, random, copy, sys, os.path, bisect, time, pprint, threading
...
class Logger:
  def __init__(self):
    self.subdir = self.unique_subdirectory_name("logs")
    print self.subdir
    os.makedirs(self.subdir)
    self.lock = threading.Lock()

  def unique_subdirectory_name(self, path):
    for i in xrange(1, 100000):
      datestring = time.strftime("%y-%m-%d", time.localtime())
      testpath = os.path.join(path, "dm-log-%s-%04d" % (datestring, i))
      if not os.path.isdir(testpath):
        return testpath
    raise "Unable to create log directory."

  def log(self, message):
    self.lock.acquire()
    try:
      logstream = open(os.path.join(self.subdir, "log.xml"), "a")
      logstream.write("<!-- %s -->\n" % (time.asctime(),))
      logstream.write("%s\n" % (message,))
      logstream.close()
    finally:
...
def slog(file, type, message):
  SLOG_LOCK.acquire()
  try:
    logout = open(file, "a")
    logout.write("%-40s %s\n" % (type, time.asctime()))
    if isinstance(message, list) or isinstance(message, tuple) or isinstance(message, dict):
      pprint.pprint(message, stream=logout, indent=4)
    else:
...
class Timer(threading._Timer):
  """Just like threading.Timer except we mark ourselves as daemon
  threads, which will not prevent Python from exiting.
  """
