...
"""
    ...
    self.assertEqual(kb.isa(logic.expr("c-action-request"), logic.expr("c-utterance")), True)
    self.assertEqual(cp_parser.slot_constraint(logic.expr("c-action-request"), logic.expr("addressee")),
                     logic.expr("c-name"))
    self.assertEqual(cp_parser.slot_constraint(logic.expr("c-action-request"), logic.expr("action")),
                     logic.expr("c-action"))
    self.assertEqual(cp_parser.slot_constraint(logic.expr("c-action-request"), logic.expr("object")),
                     logic.expr("c-thing"))
    self.assertParseEqual(cp_parser.parse("hog"), [logic.Description("c-hog")])
    self.assertParseEqual(cp_parser.parse("restart"), [logic.Description("c-restart")])
    self.assertParseEqual(cp_parser.parse("start over"), [logic.Description("c-restart")])
    self.assertParseEqual(cp_parser.parse("talkon"), [logic.Description("c-talkon")])
    self.assertParseEqual(cp_parser.parse("talk on"), [logic.Description("c-talkon")])
    self.assertParseEqual(cp_parser.parse("hog let us restart the talk on"),
                          [logic.Description("c-action-request",
                                             {"action": "c-restart",
                                              "addressee": "c-hog",
                                              "object": "c-talkon"})])
    self.assertParseEqual(cp_parser.parse("hog why don't we start over with the talk on"),
                          [logic.Description("c-action-request",
                                             {"action": "c-restart",
                                              "addressee": "c-hog",
                                              "object": "c-talkon"})])
    ...
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("ISINSTANCE(?x)")),
                                [{"?x": "i-petunia"}])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("INSTANCEOF(i-petunia, ?x)")),
                                [{"?x": "c-cat"}])
    ...
    self.assertEqual(fdl_handler.lexemes[0].phonetic_pronunciations, ["r ao jh", "r ao d"])
    self.assertEqual(fdl_handler.lexemes[1].spelling, "john")
    self.assertEqual(fdl_handler.lexemes[1].part_of_speech, "noun")
    self.assertEqual(fdl_handler.lexemes[1].phonetic_pronunciations, ["j uh n"])
