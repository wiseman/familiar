...
class TestCase(DMTestCase):
  ...
  def testConDis(self):
    ...
    kb.tell(logic.expr("Age(Cat, 35)"))
    kb.tell(logic.expr("Name(Cat, Ted)"))
    self.assertBindingsEqual(kb.ask(logic.expr("(Color(Cat, Black) & (Age(Cat, 36) | Name(Cat, Ted)))")), {})
    self.assertBindingsEqual(kb.ask(logic.expr("(Color(Cat, Black) & (Age(Cat, 36) | Name(Cat, John)))")), False)
    self.assertBindingsEqual(kb.ask(logic.expr("((Age(Cat, 36) | Name(Cat, Ted)) & Color(Cat, Black))")), {})
    self.assertBindingsEqual(kb.ask(logic.expr("((Age(Cat, 36) | Name(Cat, John)) & Color(Cat, Black))")), False)

  def testFluents(self):
    """Tests that fluents work correctly.  Fluents are propositions
    ...
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("Age(John, ?a)")), [{"?a": "40"}])
    self.assertRaises(ValueError, lambda: kb.define_fluent("Foo"))

  def testFluents2(self):
    kb = logic.PropKB()
    ...
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("PartColor(john, hair, ?a)")), [{"?a": "black"}])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("PartColor(john, hand, ?a)")), [{"?a": "green"}])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("PartColor(john, hair, brown)")), [])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("PartColor(john, hand, red)")), [])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("PartColor(john, ?x, ?y)")),
  ...
  def testInheritance(self):
    ...
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("ISA(Petunia, ?x)")),
                                [{"?x": "Petunia"}, {"?x": "Cat"}, {"?x": "Mammal"},
                                 {"?x": "Animal"}])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("ISA(Cat, ?x)")),
                                [{"?x": "Cat"}, {"?x": "Mammal"},
                                 {"?x": "Animal"}])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("ISA(Cat, Cat)")), [{}])

    # Oh, this was nasty.
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("((Bind(?x1, Cat) & Bind(?x, Painter)) & ISA(?x1, Animal)) & ISA(?x, Artist)")),
                                [{"?x": "Painter", "?x1": "Cat"}])
    ...
    # Check all_parents and all_children.
    self.assertEqual(kb.all_parents(logic.expr("Petunia")), map(lambda x: logic.expr(x), ["Petunia", "Cat", "Mammal", "Animal"]))
    self.assertEqual(kb.all_parents(logic.expr("Cat")), map(lambda x: logic.expr(x), ["Cat", "Mammal", "Animal"]))
    self.assertEqual(kb.all_parents(logic.expr("XZZY")), map(lambda x: logic.expr(x), ["XZZY"]))
    self.assertEqual(kb.all_parents(logic.expr("Animal")), map(lambda x: logic.expr(x), ["Animal"]))

    self.assertEqual(kb.all_children(logic.expr("Petunia")), map(lambda x: logic.expr(x), ["Petunia"]))
    self.assertEqual(kb.all_children(logic.expr("Cat")), map(lambda x: logic.expr(x), ["Cat", "Petunia"]))
    self.assertEqual(kb.all_children(logic.expr("XZZY")), map(lambda x: logic.expr(x), ["XZZY"]))
    self.assertEqual(kb.all_children(logic.expr("Animal")), map(lambda x: logic.expr(x), ["Animal", "Mammal", "Cat", "Petunia"]))

    self.assertEqual(kb.isa(logic.expr("Petunia"), logic.expr("Petunia")), True)
    ...
    self.assertEqual(kb.all_children(logic.expr("A")), map(lambda x: logic.expr(x), ["A"]))
    self.assertEqual(kb.all_parents(logic.expr("B")), map(lambda x: logic.expr(x), ["B"]))
    kb.tell(logic.expr("ISA(B, A)"))
    self.assertEqual(kb.all_children(logic.expr("A")), map(lambda x: logic.expr(x), ["A", "B"]))
    self.assertEqual(kb.all_parents(logic.expr("B")), map(lambda x: logic.expr(x), ["B", "A"]))
    self.assertEqual(kb.all_proper_children(logic.expr("A")), map(lambda x: logic.expr(x), ["B"]))
    self.assertEqual(kb.all_proper_parents(logic.expr("B")), map(lambda x: logic.expr(x), ["A"]))
    kb.tell(logic.expr("ISA(C, B)"))
    self.assertEqual(kb.all_proper_children(logic.expr("B")), map(lambda x: logic.expr(x), ["C"]))
    self.assertEqual(kb.all_proper_children(logic.expr("A")), map(lambda x: logic.expr(x), ["B", "C"]))
    self.assertEqual(kb.all_proper_parents(logic.expr("C")), map(lambda x: logic.expr(x), ["B", "A"]))
    
    # Check that we can query slot values correctly.
    self.assertBindingsEqual(kb.ask(logic.expr("Size(Animal, x)")), False)
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("Size(Mammal, ?s)")), [{"?s": "Medium"}])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("Size(Cat, ?s)")), [{"?s": "Medium"},])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("Size(Petunia, ?s)")), [{"?s": "Petite"},])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("Name(Animal, ?n)")), [{"?n": "Peter"},])
    self.assertBindingsEqual(kb.ask(logic.expr("Name(Cat, ?n)")), False)
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("Name(Petunia, ?n)")), [{"?n": "Petunia"}])

    # Just checking that we're substituting variables correctly.
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("(Bind(?x, Petunia) & Size(?x, ?s))")),
                                [{"?x": "Petunia", "?s": "Petite"}])
    ...
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("Size(?x, Petite)")), [{"?x": "Petunia"}])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("Size(?x, Medium)")),
                                [{"?x": "Cat"}, {"?x": "Mammal"}])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("Name(?x, Peter)")), [{"?x": "Animal"}])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("Name(?x, Petunia)")), [{"?x": "Petunia"}])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("(Bind(?x, Petite) & Size(?t, ?x))")),
                                [{"?x": "Petite", "?t": "Petunia"}])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("(Color(?x, Gray) & Size(?x, Petite))")),
                                [{"?x": "Petunia"}])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("(Color(?x, Black) & Size(?x, Petite))")),
                                [])
    # Can't have two variables in queries of inheritable slots.
    self.assertRaises(ValueError, lambda: kb.ask(logic.expr("Size(?x, ?y)")))
  ...
  def testInheritance2(self):
    ...
    # Check that we can query slot values correctly.
    self.assertBindingsEqual(kb.ask(logic.expr("Constraint(Animal, Size, foo)")), False)
    self.assertBindingsEqual(kb.ask(logic.expr("Constraint(Animal, Color, foo)")), False)
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("Constraint(Mammal, Size, ?c)")),
                                [{"?c": "SizeThing"}])
    self.assertEqual(kb.slot_value(logic.expr("Mammal"), logic.expr("Constraint"), logic.expr("Size")), logic.expr("SizeThing"))
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("Constraint(Cat, Size, ?c)")),
                                [{"?c": "SizeThing"},])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("Constraint(Petunia, Size, ?c)")),
                                [{"?c": "PetuniaSize"},])

    # Just checking that we're substituting variables correctly.
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("(Bind(?x, Petunia) & Constraint(?x, Size, ?s))")),
    ...
    # for which we haven't explicitly asserted the slot value, but
    # have the value through inheritance).
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("Constraint(?x, Size, PetuniaSize)")),
                                [{"?x": "Petunia"}])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("Constraint(?x, Color, PetuniaSize)")),
                                [])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("Constraint(?x, Size, SizeThing)")),
                                [{"?x": "Cat"}, {"?x": "Mammal"}])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("(Bind(?x, PetuniaSize) & Constraint(?t, Size, ?x))")),
    ...
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("(Color1(?x, Gray) & Constraint(?x, Size, PetuniaSize))")),
                                [{"?x": "Petunia"}])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("(Color1(?x, Black) & Constraint(?x, Size, PetuniaSize))")),
                                [])
  ...
  def testDescriptions3(self):
    ...
    # cairo.
    d = logic.Description("c-cat",
                          {"color": logic.Description("c-color", {"alternate-spelling": "grey"}),
                           "species": logic.Description("c-species",
                                                        {"origin": logic.Description("c-city",
                                                                                     {"capital": "cairo"})})})
    self.assertEqual(d.find_all(kb), [logic.expr("i-petunia")])
    self.assertEqual(d.find_instances(kb), [logic.expr("i-petunia")])
    ...
    self.assertBindingsEqual(kb.ask(logic.expr("ISA")(logic.expr(d), logic.expr("c-animal"))), {})
    self.assertBindingsEqual(kb.ask(logic.expr("ISA")(logic.expr(d), logic.expr("c-cat"))), {})
  ...
  def testInstances(self):
    ...
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("ISINSTANCE(petunia)")), [{}])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("ISINSTANCE(?x)")), [{"?x": "petunia"}])

    self.assertAllBindingsEqual(kb.ask_all(logic.expr("INSTANCEOF(petunia, ?x)")),
                                [{"?x": "cat"}])
    self.assertAllBindingsEqual(kb.ask_all(logic.expr("ISINSTANCE(?x)")), [{"?x": "petunia"}])
