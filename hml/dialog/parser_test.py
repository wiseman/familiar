...
class ConceptualParserTest(test_utils.DMTestCase):
  ...
  def testPhrasalPatternParser(self):
    ...
    self.assertEqual(p.parse_tree("[let's|let us|why don't we]"),
                     [':any',
                      [':symbol', "let's"],
                      [':sequence', [':symbol', 'let'], [':symbol', 'us']],
                      [':sequence', [':symbol', 'why'], [':symbol', "don't"], [':symbol', 'we']]])
                     
    self.assertEqual(p.parse_tree("<addressee> ?:[let's|let us|why don't we] <action> ?:[with|on] ?:the <object>"),
                     [':sequence',
                      [':slotref', [':symbol', 'addressee']],
                      [':optional',
                       [':any',
    ...
    self.assertRaises(SyntaxError, lambda: p.parse_tree("<foo"))
    self.assertRaises(SyntaxError, lambda: p.parse_tree("[foo"))
    self.assertRaises(SyntaxError, lambda: p.parse_tree("foo]"))
    self.assertRaises(SyntaxError, lambda: p.parse_tree("foo>"))
  ...
  def testSimpleParsing(self):
    ...
    self.assertParseEqual(p.parse("big red thing"), [logic.Description("BigRedThing")])
    self.assertParseEqual(p.parse("BIG red THING"), [logic.Description("BigRedThing")])
  ...
  def testSlotFilling(self):
    ...
    self.assertParseEqual(p.parse("big red thing"), [logic.Description("BigRedThing", {"color": "red"})])
    p.add_phrasal_pattern(logic.expr("BigRedThing"), "big <rudolph> thing")
    self.assertRaises(ValueError, lambda: p.parse("big crazy thing"))

    p.add_phrasal_pattern(logic.expr("BigRedThing"), "big <doesnotexist> thing")
    self.assertRaises(ValueError, lambda: p.parse("big floppy thing"))

  def testConstraintsInherited(self):
    kb = logic.PropKB()
    p = parser.ConceptualParser(kb)
    kb.tell(logic.expr("ISA(c-child, c-parent)"))
    kb.tell(logic.expr("$constraint(c-parent, color, c-color)"))
    self.assertEqual(kb.slot_value(logic.expr("c-parent"), logic.expr("$constraint"),
                                   logic.expr("color")), logic.expr("c-color"))
    self.assertEqual(kb.slot_value(logic.expr("c-child"), logic.expr("$constraint"),
                                   logic.expr("color")), logic.expr("c-color"))
    self.assertEqual(kb.ask_all(logic.expr("$constraint(c-parent, color, ?v)")),
                     [{logic.expr("?v"): logic.expr("c-color")}])
    self.assertEqual(kb.ask_all(logic.expr("$constraint(c-child, color, ?v)")),
                     [{logic.expr("?v"): logic.expr("c-color")}])
                     
  def testslotFilling2(self):
    kb = logic.PropKB()
    ...
    kb.tell(logic.expr("$constraint(c-action-request, theta-object, c-physobj)"))
    ...
    p.add_phrasal_pattern(logic.expr("c-action-request"), "<action> the <theta-object>")
    ...
    self.assertParseEqual(p.parse("small green thing"),
                          [logic.Description("c-physobj", {"color": "c-green", "size": "c-small"})])
  ...
  def testSequence(self):
    kb = logic.PropKB()
    p = parser.ConceptualParser(kb)
    p.add_phrasal_pattern_object(logic.expr("c-thing"), [":sequence", "a", "b", "c"])
    ...
    p.add_phrasal_pattern_object(logic.expr("c-thing"), [":sequence", "a", [":sequence", "x", "y", "z"]])
    p.add_phrasal_pattern_object(logic.expr("c-thing"), [":sequence", "a", [":sequence", "x"]])
    p.add_phrasal_pattern_object(logic.expr("c-thing"), [":sequence", [":sequence", "x", "y", "z"], "a"])
    ...
    p.add_phrasal_pattern_object(logic.expr("c-thing"), [":sequence", [":sequence", "x", 0, "z"], "a"])
    self.assertRaises(TypeError, lambda: p.parse_tokens(["x", 0, "z", "a"]))

  def testOptional(self):
    kb = logic.PropKB()
    p = parser.ConceptualParser(kb)
    p.add_phrasal_pattern_object(logic.expr("c-thing"), [":sequence", "the", [":optional", "big"], "thing"])
    self.assertParseEqual(p.parse("the thing"), [logic.Description("c-thing")])
    self.assertParseEqual(p.parse("the big thing"), [logic.Description("c-thing")])
    self.assertParseEqual(p.parse("the"), [])
    self.assertParseEqual(p.parse("thing"), [])

    p = parser.ConceptualParser(kb)
    p.add_phrasal_pattern_object(logic.expr("c-thing"), [":sequence", [":optional", "the"], "thing"])
    self.assertParseEqual(p.parse("thing"), [logic.Description("c-thing")])
    self.assertParseEqual(p.parse("the thing"), [logic.Description("c-thing")])

    p = parser.ConceptualParser(kb)
    p.add_phrasal_pattern_object(logic.expr("c-thing"), [":sequence", "the", [":optional", "thing"]])
  ...
  def testAny(self):
    kb = logic.PropKB()
    p = parser.ConceptualParser(kb)
    p.add_phrasal_pattern_object(logic.expr("c-thing"), [":sequence", "a", [":any", "b", "c", "d"], "e"])
  ...
  def testHeadDirective(self):
    ...
    p.add_phrasal_pattern(logic.expr("c-feed"), "feed")
    p.add_phrasal_pattern(logic.expr("c-action-request"), "<action> <theta-object>")
    p.add_phrasal_pattern(logic.expr("c-action-request"), "<action> the <theta-object>")
    ...
    self.assertParseEqual(p.parse("feed the small green cat"),
                          [logic.Description("c-action-request",
                                             {"action": "c-feed",
                                              "theta-object": logic.Description("c-cat",
                                                                                {"color": "c-green",
                                                                                 "size": "c-small"})})])
    self.assertParseEqual(p.parse("feed the small green dog"),
                          [logic.Description("c-action-request",
                                             {"action": "c-feed",
                                              "theta-object": logic.Description("c-dog",
                                                                                {"color": "c-green",
                                                                                 "size": "c-small"})})])
  ...
  def testExamples(self):
    ...
    p.add_phrasal_pattern(logic.expr("c-action-request"),
                          "?:<addressee> ?:[let's|let us|lets|why don't we] <action> ?:[with|on] ?:the <object>")
  ...
  def testExamples2(self):
    ...
    kb.tell(logic.expr("$constraint(c-action-request, object, c-thing)"))
    p = parser.ConceptualParser(kb)
    pp = parser.PhrasalPatternParser()
    p.add_phrasal_pattern(logic.expr("c-hog"), "hog")
    p.add_phrasal_pattern(logic.expr("c-start-over"), "start [over|again]")
    p.add_phrasal_pattern(logic.expr("c-start-over"), "restart")
    p.add_phrasal_pattern(logic.expr("c-nine-line"), "[nine|9] line")
    ...
    self.assertParseEqual(p.parse("9 line"), [logic.Description("c-nine-line")])
    self.assertParseEqual(p.parse("start again"), [logic.Description("c-start-over")])
    self.assertParseEqual(p.parse("hog, let's start over with the nine-line"),
                          [logic.Description("c-action-request",
                                             {"addressee": "c-hog",
                                              "action": "c-start-over",
...
#                                             {"value": 7})])
  ...
  def testFillSlotsAtMostOnce(self):
    kb = logic.PropKB()
    p = parser.ConceptualParser(kb)
    ...
    self.assertParseEqual(p.parse("red thing"), [logic.Description("c-thing", {"color": "c-red"})])
...
class ICPTest(test_utils.DMTestCase):
  ...
  def testIndexSetPatternParser(self):
    ...
    set = p.parse(None, "big dog")
    self.assertEqual(set.indices, ["big", "dog"])
    self.assertEqual(set.required_indices, [])

    set = p.parse(None, "big $dog")
    self.assertEqual(set.indices, ["big", logic.expr("dog")])
    self.assertEqual(set.required_indices, [])

    set = p.parse(None, "!big !dog")
    self.assertEqual(set.indices, ["big", "dog"])
    self.assertEqual(set.required_indices, ["big", "dog"])

    set = p.parse(None, "!big black !$dog")
    self.assertEqual(set.indices, ["big", "black", logic.expr("dog")])
    self.assertEqual(set.required_indices, ["big", logic.expr("dog")])
    ...
    set = p.parse("c-dog", "!{size} {color} dog")
    self.assertEqual(set.target_concept, logic.expr("c-dog"))
    self.assertEqual(set.indices, [logic.expr("c-size"), logic.expr("c-color"), "dog"])
    self.assertEqual(set.required_indices, [logic.expr("c-size")])
    self.assertEqual(set.slots, [["size", logic.expr("c-size")], ["color", logic.expr("c-color")]])
    ...
    self.assertRaises(SyntaxError, lambda: p.parse("c-dog", "{!}"))
    self.assertRaises(SyntaxError, lambda: p.parse("c-dog", "{foo} {foo}"))
    self.assertEqual(p.parse(None, ""), parser.IndexSet())
    self.assertEqual(p.parse(None, "   "), parser.IndexSet())


  def testICPWithLiteralTokens(self):
  ...
  def testICPSlotFilling(self):
    ...
    self.assertEqual(len(results), 1)
    self.assertParseEqual(results,
                          [logic.Description("c-dog", {"size": "c-big", "color": "c-red"})])
    
    results = p.parse("big dog")
    self.assertEqual(len(results), 1)
    self.assertParseEqual(results, [logic.Description("c-dog", {"size": "c-big"})])
