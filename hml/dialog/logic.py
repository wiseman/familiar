...
#            if is_heritable and some(is_variable, query.form()[0:-1]):
...
#        found_bindings = False
  ...
  def filter_possible_matches(self, match_fn, possible_matches, query, active_bindings):
    ...
    if possible_matches == None:
      ...
      next = possible_matches.next()
      new_bindings = match_fn(query, next, active_bindings)
    ...
    if is_var_symbol(slot_value.op) and is_var_symbol(thing.op):
      ...
      for answer in answers:
        for child in self.all_children(answer):
          ...
          if child_slot_value == None or child_slot_value == slot_value:
  ...
  def all_parents(self, x):
    """Returns a list of all parents of x, from most specific to least
    specific.
    """
    # Check the memoization cache first.
    if x in self.parent_cache:
    ...
    self.parent_cache[x] = parents
    return parents

  def all_proper_parents(self, x):
    return self.all_parents(x)[1:]

  # Cache for memoizing all_children.
  child_cache = {}

  def all_children(self, x):
    """Returns a list of all children of x."""
    # Check the memoization cache first.
    if x in self.child_cache:
  ...
  def all_proper_children(self, x):
    """Returns a list of all proper children of x, i.e. the list
    does not contain x itself.
    """
    return self.all_children(x)[1:]
...
class ISAFunction(MemoryFunction):
  ...
  def direct_children(self, kb, concept):
    v = expr("?x")
    isaexpr = expr("$ISA")(v, concept)
    for s in kb.ask_generator(isaexpr):
      yield s[v]

  def direct_parents(self, kb, concept):
    v = expr("?x")
    isaexpr = expr("$ISA")(concept, v)
    for s in kb.ask_generator(isaexpr):
      yield s[v]

  def all_children(self, kb, concept):
    yield concept
    for child in self.direct_children(kb, concept):
      for subchild in self.all_children(kb, child):
        yield subchild

  def all_parents(self, kb, concept):
    yield concept
    for parent in self.direct_parents(kb, concept):
      for superparent in self.all_parents(kb, parent):
        yield superparent
