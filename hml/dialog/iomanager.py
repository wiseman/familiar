...
<table>
  ...
  def say(self, who, string):
    ...
    sys.stdout.write(
      '<td class="pilot">%s</td></tr>\n' % (cgi.escape(string),))

  def debug(self, string):
    if self.enable_debug:
      sys.stdout.write(
        '<tr><td colspan="3" class="debug">%s</td></tr>\n' % (
        cgi.escape(string),))
  ...
  def echo_input(self, string):
    ...
    sys.stdout.write(
      '<tr><td class="jtacline">%s</td><td class="to">%s:</td>'
      '<td class="jtac">%s</td></tr>\n' % (
...
class ConsoleIOManager:
  ...
  def getline(self):
    ...
    def read_line():
      if self.file_input_stream is not None:
        # using file for canned transcript test.  ignore lines
        # starting with #. (mrh 2/20/07)
        line = self.file_input_stream.readline()
        if line == '':
          raise EOFError
  ...
  def execute_command(self, line):
    ...
    if command == '%EXPECT':
      ...
      raise Error('Bad command %r in input file' % (line,))
...
# Currently, test scores are tallied in the wrong place (in the
# TestSuite, not in the individual test objects), which means doesn't
# spit out nice error lists at the end, but it's much easier here to
# look back through the output and see what went wrong.  Putting
# errors in the right place isn't hard.
...
class TestIOManager:
  ...
  <testsuite name="descriptive name">
    ...
    <test name="test-name">
      ...
      <output [should-fail="True|False"]>
        expected output, may include regexp
      </output>
    </test>*
  </testsuite>
  """
  ...
  def debug(self, text):
    # whether or not to test debug output is a very interesting
    # question, I think.  it seems valuable, but let's not start with
    # it.
    if self.enable_debug:
      sys.stdout.write("[debug: %s]\n" % (text,))
  ...
  def shutdown(self):
    ...
    if num_tests_run != len(self.test_suite.tests):
      print "  #\n  # tests not run: %s" % (
        len(self.test_suite.tests) - num_tests_run,)
  ...
  class TestSuite(object):
    ...
    def __init__(self, file_path, dialog_manager):
      ...
      # condition variable for waiting until input is available
      self.input_lock = threading.Condition()
      # is the current test done for the moment?
      self.input_ready = True

      self.tests = []
      # what are they python guidelines about doing lots of
      # computation in the constructor?  no matter for now, just load
      # up stuff.
      self.load_tests()

    def next_output(self, text):
      # The pilot has said something, handle it.  More or less assume
      # that outputs govern when one test is done and the next should
      # be set up.
      if self.current_test_idx >= len(self.tests):
        logging.warn(
          'Still getting output but we\'re out of tests: {%s}', text)
        # bail
        return

      test_obj = self.current_test()
      # for WingIDE context help
      assert(isinstance(test_obj, TestIOManager.TestNode))
      try:
        # logging.debug(
        #   'test %d, (%d,%d,%d), text to match: {%s}',
        #   self.current_test, self.success_count, self.failure_count,
        ...
        self.print_test_exception(
          'Test %s failed with an exception when matching output.' % (
            self.name,),
          sys.exc_info())
    ...
    def next_input(self):
      ...
      in special cases.
      """
      # if the previous test is finished (output all matched or
      # failed), get the new test
      self.wait_for_input_ready()

      if self.current_test_idx >= len(self.tests):
        # not an error, this will normally happen.  raise EOFError to
        # signal we're done.
        raise EOFError
      ...
      if self.current_test().setup_code is not None:
        try:
          ...
          self.print_test_exception(
            'Test %s failed with an exception in the setup code.' % (
              self.name,),
            sys.exc_info())
    ...
    def wait_for_input_ready(self, timeout=None):
      ...
      try:
        try:
          ...
          while (not self.input_ready and
                 ((not too_late) or
                  time.time() >= too_late)):
    ...
    def load_tests(self):
      ...
      # print xmldoc.toxml()
      # there should only be one child, <testsuite>.  (err, why?  I
      # suppose we could have multiple suites per file...)
      ...
      for node in xmldoc.childNodes:
        if isinstance(node, minidom.Element):
          ...
          if node.nodeName == "test":
            ...
            raise Error('Unknown node type: %s' % (node.nodeName,))
    ...
    def handle_include_node(self, unused_node):
  ...
  class TestNode:
    ...
    def __init__(self, xml_node):
      ...
      logging.debug(
         'success loading test node %s, input = %s, output = %s, setup = %s',
        ...
        if isinstance(tmp_node, minidom.Element):
          if tmp_node.nodeName == "input":
            if self.input is not None:
              raise Error('Extra input node in ' + node.toxml())
            ...
            if tmp_node.hasAttribute("should-fail"):
              self.should_fail = (
                tmp_node.attributes["should-fail"].value == "True")
          elif tmp_node.nodeName == "setup":
            if self.setup_code is not None:
              raise Error('Extra setup node in ' + node.toxml())
            self.setup_code = tmp_node.firstChild.nodeValue
          else:
            raise Error('Extra unknown node in ' + node.toxml())
...
#        print "Outputs for test %s, in order: %s" % (self.name, self.outputs)
    ...
    def match_output(self, text):
      """Does the new utterance match the expected output?  Return
      True/False."""

      logging.debug('match text = {%s}, current_output = %d, output list = %s',
                    text, self.current_output, self.outputs)
      ...
      if result == False:
        ...
        print "########################################"
        print ""
        # new thought: let's try saying that if there's an error in
        # any of the the expected output phrases for a given test,
        # then the test is complete.  that may help the system shut
        # down when a multi-output test fails.
    ...
    def is_complete(self):
      """Return True if this test has run through all expectations,
      false otherwise."""
      return self.is_complete_flag

    def get_input(self):
      """Return the input string for this test."""
      # tests will almost always have input; maybe the first test in a
      # set is an exception.  or! tests where the <setup> element
      # causes some event to be triggered. (mrh 4/25/07)
      self.is_complete_flag = (self.current_output >= len(self.outputs))
      return self.input
...
class Lexeme:
  ...
  def __init__(self, spelling, part_of_speech):
    ...
    if not part_of_speech in self.PARTS_OF_SPEECH:
      raise ValueError('Part of speech is %s, but must be one of %s.' % (
