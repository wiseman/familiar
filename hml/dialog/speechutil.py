...
class MicrosoftSRGrammarGenerator:
    ...
    def tsort_rules(self, rules):
        ...
        # assert len(result) == len(self.rules), "Apparent cycle in rules.  See grammar_debug.txt for details."
        if not len(result) == len(self.rules):
            self.output_debug_info(self.rules, result)
            print "\n\nWARNING: Apparent cycle in rules.  Not throwing assertion.  See grammar_debug.txt for details."
        return result
    ...
    def output_debug_info(self, orig_list, sorted_list):
        ...
        out.write("Debugging problem where original rule set had %d entries, and sorted set has %d\n" % (len(orig_list), len(sorted_list)))

        only_in_orig = []
        for name in orig_list:
            if (name not in [elem.name for elem in sorted_list if elem.name == name]):
    ...
    def generate_grammar(self):
        ...
        sys.stdout.write("\n")
        sys.stdout.write("Grammar has %s rules.\n" % (len(self.rules),))
        return self.to_xml()
